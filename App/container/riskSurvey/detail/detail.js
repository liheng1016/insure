...
export class ContentGeneral extends Component{
		...
		return (
			<ul className={style["general"]} >
				<div className={style["general-title"]}>
					<div className={style["general-company"]}>{risk.task_name}</div>
					<div className={style["general-score"]}>
						<span className={style["score"]}>{risk.final_score}分</span>

							{
								this.star_img(risk.final_score)
							}
				...
				<LiGeneral LableName={"创建人 : "}
					Content={risk.people_name}/>
				<LiGeneral LableName={"排查人 : "}
					Content={risk.user_name}/>
				<LiGeneral LableName={"排查表 : "}
					Content={risk && risk.form && risk.form.formName}/>
				<LiGeneral LableName={"查勘时间 : "}
				...
				<LiGeneral LableName={"整改建议 : "}
					Content={risk.remark}/>
				<li className={style["source_warp"]}>
					来源 : {risk.organ&& risk.organ.organName}
				</li>
			</ul>
		);
	}
	...
	star_img(score){
		...
		let scoreStars=0,{maxStars} = this.state;
		...
		let fullStars = this.getNewArray(scoreStars),
			emptyStars = this.getNewArray(maxStars - scoreStars);

		return(
			<span className={style["star_warp"]}>
				{
					fullStars.map((k,index)=>
						<img src={require("../img/check_star.png")} key={index}/>
					)
				}
				{
					 emptyStars.map((k,index)=>
						<img src={require("../img/star.png")} key={index}/>
					)	
				}
			</span>
...
export class DangerConter extends Component{
	...
	getContent(){
		...
		let ButtonStyle = {width:'92px',background:'#0493df',float:'right',position:'absolute',bottom:'15px',right:'3%'};
			...
			return(
				<div className={style["dangercontent"]} key={key}>
					...
					<div className={style["solve-picture"]}>
						{
							item.is_feedback == 0?
							<img src={require("../img/no-feek.png")}/>:
							<img src={require("../img/feek.png")}/>
						}
					</div>
					<Button styleCss={ButtonStyle} text={'风险跟踪'} onClick={(e)=>this.go_to_detail(item)}/>
				</div> 
			)	
		})
	}
	go_to_detail(item){
		this.context.router.push("/riskDetailedList/detail/"+item.hidden_danger_id)
	}
	getDangerPicture(img=[]){
		...
		return img.map((m,key)=>{
			return(
				<li className={style["pic--li"]}  key={key} onClick={e=>big_img(img,key)}>
